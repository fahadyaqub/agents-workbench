...
- `[2026-05-28] [engineering] [confirmed]` Eval-driven projects need a running development journal, not just a changelog. Every eval run that surfaces a finding — and every fix that results from it — should be written up in prose: what the agent did, why it went wrong, what the root cause was, and what was changed. This serves as institutional memory for the next person (or agent) building something similar. Keep it in `local/memory/` for the project. The journal entry should describe the thinking, not just the outcome — "the tool name implied X so the agent did X" is more useful than "fixed overlay behavior."

- `[2026-05-29] [global] [confirmed]` The orchestrator-worker pattern should be the default pre-flight for execution-heavy work, not for every user message. When an agent can decompose a task into 3+ atomic, independent subtasks with binary completion criteria and no sensitive context leakage, delegating to cheaper parallel workers can compound over time: faster execution, lower cost, and a learning loop that improves calibration with each run. The orchestrator's job is decomposition quality and result review — not integration delegation. See `shared/workflows/agent-runner.md`.

- `[2026-05-29] [global] [confirmed]` Every substantive agent-runner run, including meaningful fast-path bypass decisions, must produce a structured eval or learning entry. Unrecorded runner decisions are wasted signal, but trivial conversations should not create noise. The learnings file is the compounding asset — a workflow that doesn't learn degrades into overhead. Record: task count, accept rate, escalation rate, time saved vs inline, and a one-sentence verdict when a run actually occurs. Extract durable lessons into the fast-path rules table.

- `[2026-05-19] [engineering] [confirmed]` Bug investigations must be grounded independently, never built around the user's framing. (1) Form your own hypothesis from the code first. (2) Use the user's assumption only as corroboration. (3) If no direct link is found, say so explicitly — do not invent a bug to fill the gap. (4) If an unrelated issue is found while investigating, flag it clearly as separate. (5) If your finding conflicts with the user's, recheck and give a final verdict. The user can be wrong about the cause of a symptom; confirming a wrong assumption wastes everyone's time and erodes trust.
