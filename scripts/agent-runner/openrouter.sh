...
if [ "$#" -ne 2 ]; then
  echo "usage: scripts/agent-runner/openrouter.sh <model> <task-spec.md>" >&2
  exit 64
fi
...
python3 - "$MODEL" "$SPEC_FILE" <<'PY' | curl -sS --fail-with-body https://openrouter.ai/api/v1/chat/completions \
